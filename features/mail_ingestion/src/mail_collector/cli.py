...
from .oauth import DEFAULT_OUTLOOK_CLIENT_ID, begin_device_flow, complete_device_flow
...
def command_outlook_auth(args: argparse.Namespace) -> int:
    ...
    flow = begin_device_flow(client_id, tenant)
    print(json.dumps({
        "ok": True,
    ...
    complete_device_flow(client_id, tenant, flow, settings.oauth2_token_cache, expected_username=settings.username)
