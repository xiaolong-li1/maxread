...
本节必须覆盖：
- 先用一个短段交代方法设定与上下文：输入是什么、输出是什么、任务对象、必要假设、关键状态/符号，以及前一节的瓶颈为什么导出这套设计。不要让读者在没有上下文时直接撞上模块名和公式。
- 再按原文小节或执行顺序展开核心模块。每个模块都说明“为什么需要它 -> 接收什么 -> 如何计算/执行 -> 产出什么并交给下一步 -> 它具体缓解哪个瓶颈”，形成连续因果链，而不是模块清单。
- 关键公式逐条解释：公式从哪个步骤产生，变量含义、计算顺序、形状或取值范围，以及它解决哪个问题。重要公式后必须补一句直觉，并说明它与前后公式/模块的关系。
- 如果有算法/训练流程/推理流程，按原文步骤复述，不要省略条件、阈值、采样策略、损失项。
- 明确区分训练阶段与推理/部署阶段；如果论文给了一个请求、样本或 program 的完整流转过程，用一个简短端到端例子串起各模块。
...
写作要求：
- 篇幅目标：2600-4200 中文字。材料充分时，方法节通常占正文的 35%-50%，但不要靠重复或补写原文没有的细节凑长度。
