...
def intro_message(feedback_url: str = "") -> str:
    return (
        ...
        "- Hugging Face 模型仓库、GitHub 技术报告 PDF 链接\n"
        "- 部分普通网页文章链接\n\n"
        "一条消息可以放多篇，我会并行处理并提示排队顺序。\n"
        "私聊我可以直接发链接；群聊里必须 @ 我，我才会处理。\n\n"
        f"也可以使用网页项目台：{WEB_PROJECTS_URL}\n"
        "网页里每篇论文是一张持续更新的项目卡，可以看进度、打开文档、重试、收藏和整理分类。绑定后，分类筛选旁可点“新建分类”创建个人分类；自动整理可以使用这些分类，但不会擅自新建分类或覆盖手动分类。\n"
        ...
        "暂不稳定/暂不支持：微信链接、未加入安全白名单的任意 PDF URL、纯文本问答；SVG 网页装饰图会跳过，避免飞书里出现裁剪或大白边。"
    )
...
def group_intro_message() -> str:
    return (
        ...
        "网页里可以看每篇论文的实时项目卡、打开文档、重试和整理分类。绑定后可点分类筛选旁的“新建分类”创建个人分类；自动整理不会覆盖手动分类。绑定方法：点网页右上角“游客”，复制绑定指令，回飞书私聊我发送，再返回网页等待姓名自动出现。\n\n"
        "目前支持：\n"
        "- arXiv ID / abs / PDF 链接\n"
        ...
        "- 暂不稳定/暂不支持：微信链接、未加入安全白名单的任意 PDF URL、纯文本问答；SVG 网页装饰图会跳过。\n"
