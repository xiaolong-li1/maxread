...
Base also stores standalone `院校`, `排名`, and `排名依据` fields plus
`是否985`、`是否C9`、`是否已回复`、`来源邮箱`. AI extracts the school,
academic metrics, rank, and a verbatim rank-evidence excerpt. Production code
must not infer ranks with regex or numeric heuristics. Official deterministic
lists assign institution tags, and outgoing thread direction determines reply
