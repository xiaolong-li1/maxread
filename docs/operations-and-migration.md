...
阿里云仍是唯一数据库、飞书监听器和网页入口；论文计算由 `ziplab-5090` 的远端 paper worker 承担，网页文章继续在阿里云处理。5090 不得启动第二个同一飞书应用的消息监听器，只运行受限 SSH 隧道和 `paper-worker`。详见 `docs/remote-paper-worker.md`。旧目录 `/opt/maxread-stage-20260821` 只作为历史备份，迁移完成并核对数据前不要删除。
